...
def save_search(domain, expires, date_available, user_id, anon=False):
    from app.blueprints.api.models.searched import SearchedDomain

    if not anon and not db.session.query(exists().where(and_(SearchedDomain.name == domain, SearchedDomain.user_id == user_id))).scalar():
        s = SearchedDomain()
        s.name = domain
        s.expires = expires
